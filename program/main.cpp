  ...
  if ((argc > 1) && (argv != NULL))
  {
    ...
    while (i < argc)
    {
      if (argv[i] != NULL)
      {
        ...
        else if ((param == "--no-ownership") || (param == "--no-chown"))
        {
          adjustOwnership = false;
        }
        else if ((param == "--no-permission") || (param == "--no-permissions") || (param == "--no-chmod"))
        {
          adjustPermissions = false;
        }
        else if ((param == "--ownership-only") || (param == "--chown-only"))
        {
          adjustPermissions = false;
          adjustOwnership = true;
        }
        else if ((param == "--permissions-only") || (param == "--chmod-only"))
        {
          adjustPermissions = true;
          adjustOwnership = false;
        }
        else if ((param == "--dry-run") || (param == "-n") || (param == "--simulate") || (param == "-s"))
        {
          if (!hasForceOrDryRun)
          {
            dryRun = true;
          ...
          {
            std::cerr << "Error: Parameters --force and --dry-run are mutually exclusive and may only be given once per run.\n";
            return rcInvalidParameter;
          }
        }
        else if ((param == "--force") || (param == "-f"))
        {
          if (!hasForceOrDryRun)
          {
            dryRun = false;
        ...
        else if (param == "--save")
        {
          if (save)
          {
            std::cerr << "Error: Parameter --save may only be given once per run.\n";
            return rcInvalidParameter;
          }
          if (restore)
          {
            std::cerr << "Error: Parameters --save and --restore are mutually exclusive.\n";
            return rcInvalidParameter;
          }
          save = true;
        } // if --save
        else if (param == "--restore")
        {
          if (restore)
          {
            std::cerr << "Error: Parameter --restore may only be given once per run.\n";
            return rcInvalidParameter;
          }
          if (save)
          {
        ...
        else if (sourceDir.empty())
        {
          sourceDir = param;
          if (!restore)
            std::cerr << "Source directory was set to \"" << sourceDir << "\".\n";
          else
